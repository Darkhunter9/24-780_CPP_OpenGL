/*
24-780-B Engineering Computation Problem Set 9 - Bitmap Editor
Name: Zihao Ding
Andrew ID: zihaod
Date: 10/29/2018
Description:
...
{
	create(incoming.getWidth(), incoming.getHeight());
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			pixels[i*width + j] = incoming.getPixel(j, i);
		}
	}
}
...
template <class T>
void BitmapTemplate<T>::cleanUp() // used to reset and free up all space used by object
{
...
template <class T>
void BitmapTemplate<T>::create(int w, int h) // initializes object with given dimensions
{
	width = w;
	height = h;
...
template <class T>
void BitmapTemplate<T>::setPixel(int x, int y, const T &p) // sets the pixel value
{
	pixels[y * width + x] = p;
}

template <class T>
T BitmapTemplate<T>::getPixel(int x, int y) const // returns the value/color of the pixel
{
